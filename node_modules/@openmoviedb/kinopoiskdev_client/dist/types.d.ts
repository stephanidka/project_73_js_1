...
declare class ClientRequest {
    ...
    get<T, P extends Record<string, unknown>>(version: VERSION, path: string, params?: P): Promise<IResponse<T>>;
}
/**
 *
 * @export
...
export interface InlineResponse200 {
     ...
     */
    info?: {
        [key: string]: InlineResponse200Info;
    };
    /**
     *
     * @type {{ [key: string]: InlineResponse200Info; }}
...
export type AllFields<T extends IQueryFields> = T['BooleanFields'] | T['NumberFields'] | T['DateFields'] | T['StringFields'];
export type SelectFields<T extends IQueryFields> = AllFields<T> | T['ObjectFields'];
export type FiltersValue = string | number | boolean | SPECIAL_VALUE | (string | number | boolean | SPECIAL_VALUE)[];
export type Filter<T extends IQueryFields> = Partial<{
    [key in AllFields<T> | 'selectFields' | 'sortField' | 'sortType' | 'page' | 'limit']: FiltersValue;
}>;
...
export interface IQueryBuilder<T extends IQueryFields> {
    ...
    filterExact(field: AllFields<T>, value: string | number | boolean | SPECIAL_VALUE): IQueryBuilder<T>;
    filterRange(field: T['NumberFields'], range: [number, number]): IQueryBuilder<T>;
    filterDateRange(field: T['DateFields'], range: [Date, Date]): IQueryBuilder<T>;
...
export interface IReviewService {
    getByFilters(filters: Filter<ReviewFields>): Promise<IResponse<ReviewDocsResponseDto>>;
}
...
export interface ISeasonService {
    getByFilters(filters: Filter<SeasonFields>): Promise<IResponse<SeasonDocsResponseDto>>;
}
declare class SeasonService implements ISeasonService {
    constructor(request: ClientRequest);
    getByFilters(filters: Filter<SeasonFields>): Promise<IResponse<SeasonDocsResponseDto>>;
}
...
export interface MovieFields extends IQueryFields {
    StringFields: 'name' | 'alternativeName' | 'enName' | 'names.name' | 'externalId.kpHD' | 'externalId.imdb' | 'type' | 'tagline' | 'persons.name' | 'persons.enName' | 'persons.description' | 'description' | 'shortDescription' | 'slogan' | 'status' | 'ratingMpaa' | 'logo.url' | 'poster.url' | 'backdrop.url' | 'videos.trailers.url' | 'videos.trailers.site' | 'videos.trailers.type' | 'genres.name' | 'countries.name' | 'persons.photo' | 'budget.value' | 'budget.currency' | 'watchability.items.name' | 'audience.country';
    DateFields: 'premiere.world' | 'premiere.russia' | 'premiere.digital' | 'premiere.bluray' | 'premiere.dvd';
    NumberFields: 'id' | 'similarMovies.id' | 'sequelsAndPrequels.id' | 'externalId.tmdb' | 'typeNumber' | 'movieLength' | 'seriesLength' | 'totalSeriesLength' | 'year' | 'rating.kp' | 'rating.imdb' | 'rating.tmdb' | 'rating.filmCritics' | 'rating.russianFilmCritics' | 'rating.await' | 'votes.kp' | 'votes.imdb' | 'votes.tmdb' | 'votes.filmCritics' | 'votes.russianFilmCritics' | 'votes.await' | 'ratingAgeLimits' | 'ageRating' | 'persons.id' | 'budget.value' | 'fees.world.value' | 'fees.russia.value' | 'fees.usa.value' | 'image.postersCount' | 'image.backdropsCount' | 'image.framesCount' | 'reviewInfo.count' | 'reviewInfo.positiveCount' | 'seasonsInfo.number' | 'seasonsInfo.episodesCount' | 'videos.trailers.size' | 'videos.teasers.size' | 'audience.count' | 'top10' | 'top250' | 'releaseYears.start' | 'releaseYears.end';
    BooleanFields: 'ticketsOnSale' | 'isSeries';
    ObjectFields: 'externalId' | 'image' | 'reviewInfo' | 'seasonsInfo' | 'videos' | 'audience' | 'releaseYears' | 'budget' | 'fees' | 'watchability' | 'persons' | 'rating' | 'genres' | 'countries' | 'similarMovies' | 'sequelsAndPrequels' | 'poster' | 'backdrop' | 'logo' | 'names' | 'facts';
}
export interface IMovieService {
    getById(id: number): Promise<IResponse<MovieDtoV13>>;
    getRandom(): Promise<IResponse<MovieDtoV13>>;
    getByFilters(filters: Filter<MovieFields>): Promise<IResponse<MovieDocsResponseDtoV13>>;
    getBySearchQuery(filters: SearchFilter): Promise<IResponse<SearchMovieResponseDto>>;
    getAwardsByFilters(filters: Record<string, string>, paginationParams?: IPagination): Promise<IResponse<MovieAwardDocsResponseDto>>;
    getPossibleValuesByField(field: string): Promise<IResponse<PossibleValueDto[]>>;
}
declare class MovieService implements IMovieService {
    constructor(request: ClientRequest);
    getById(id: number): Promise<IResponse<MovieDtoV13>>;
    getRandom(): Promise<IResponse<MovieDtoV13>>;
    getByFilters(filters: Filter<MovieFields>): Promise<IResponse<MovieDocsResponseDtoV13>>;
...
export interface PersonFields extends IQueryFields {
    StringFields: 'name' | 'enName' | 'photo' | 'sex' | 'birthPlace.value' | 'deathPlace.value' | 'spouses.name' | 'spouses.divorced' | 'spouses.divorcedReason' | 'spouses.sex' | 'spouses.children' | 'spouses.relation' | 'profession.value' | 'facts.value' | 'movies.name' | 'movies.alternativeName' | 'movies.description' | 'movies.enProfession';
    DateFields: 'birthday' | 'death';
    NumberFields: 'id' | 'movies.id' | 'movies.rating' | 'age' | 'countAwards' | 'growth' | 'spouses.id' | 'spouses.children' | 'spouses.name';
    BooleanFields: never;
    ObjectFields: 'movies' | 'spouses' | 'facts' | 'profession' | 'birthPlace' | 'deathPlace';
}
export interface IPersonService {
    getById(id: number): Promise<IResponse<Person>>;
    getByFilters(filters: Filter<PersonFields>): Promise<IResponse<PersonDocsResponseDto>>;
    getBySearchQuery(filters: SearchFilter): Promise<IResponse<SearchPersonResponseDto>>;
    getAwardsByFilters(filters: Record<string, string>, paginationParams?: IPagination): Promise<IResponse<PersonAwardDocsResponseDto>>;
}
declare class PersonService implements IPersonService {
    constructor(request: ClientRequest);
    getById(id: number): Promise<IResponse<Person>>;
    getByFilters(filters: Filter<PersonFields>): Promise<IResponse<PersonDocsResponseDto>>;
    getBySearchQuery(filters: SearchFilter): Promise<IResponse<SearchPersonResponseDto>>;
...
export interface IKeywordService {
    getByFilters(filters: Filter<KeywordFields>): Promise<IResponse<KeywordDocsResponseDto>>;
}
declare class KeywordService implements IKeywordService {
    constructor(request: ClientRequest);
    getByFilters(filters: Filter<KeywordFields>): Promise<IResponse<KeywordDocsResponseDto>>;
}
export interface IImageService {
    getByFilters(filters: Record<string, string>): Promise<IResponse<ImageDocsResponseDto>>;
}
declare class ImageService implements IImageService {
    constructor(request: ClientRequest);
    getByFilters(filters: Record<string, string>): Promise<IResponse<ImageDocsResponseDto>>;
}
...
export interface IStudioService {
    ...
    getByFilters(filters: Filter<StudioFields>): Promise<IResponse<StudioDocsResponseDto>>;
}
declare class StudioService implements IStudioService {
    constructor(request: ClientRequest);
    getById(id: number): Promise<IResponse<Studio>>;
    getByFilters(filters: Filter<StudioFields>): Promise<IResponse<StudioDocsResponseDto>>;
}
...
export abstract class QueryBuilder<T extends IQueryFields> implements IQueryBuilder<T> {
    protected params: any;
    constructor();
    select(fields: SelectFields<T>[]): this;
    sort(field: AllFields<T>, sortType: SORT_TYPE | '1' | '-1'): this;
    filterExact(field: AllFields<T>, value: string | number | boolean | SPECIAL_VALUE): this;
